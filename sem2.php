...
$P = new MySemester("Semestr II", "Tymoteusz Surynt");
...
$s .= $P->startContent();
$s .= $P->getPanel();
$s .= $P->getHeaderSection(true, "Semestr II");
$s .= $P->startRow();

if (!($stmt = $mysqli->prepare("SELECT id,title,link,1_title,1_card,1_ol,1_bonus,2_title,2_card,2_ol,2_bonus FROM semesters  WHERE sem_num LIKE 2"))) {
    die ();
}
if (!$stmt->execute()) {
    die ();
...
$s.=$P->addComments($select,"send.php","sem2.php");
$s .= $P->stopDiv(3);
