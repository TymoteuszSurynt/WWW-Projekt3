...
$P = new MySemester("Semestr V", "Tymoteusz Surynt");
...
$s .= $P->startContent();
$s .= $P->getPanel();
$s .= $P->getHeaderSection(true, "Semestr V");
$s .= $P->startRow();

if (!($stmt = $mysqli->prepare("SELECT id,title,link,1_title,1_card,1_ol,1_bonus,2_title,2_card,2_ol,2_bonus FROM semesters  WHERE sem_num LIKE 5"))) {
    die ();
}
if (!$stmt->execute()) {
    die ();
