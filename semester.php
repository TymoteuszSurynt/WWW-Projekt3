...
if(isset($_GET['semesterNumber'])&& isset($_GET['semesterName']) && !empty($_GET['semesterNumber'])&& !empty($_GET['semesterName'])){
    $semesterNumber=$_GET['semesterNumber'];
...
if($result4 = $res4->fetch_assoc()){
    $s=$P->addComments($select,"send.php","semester.php?semesterNumber=".$semesterNumber."&semesterName=".$semesterName,[$num,$result4['option']]);
}
$s.=$P->stopDiv(3);
